/**
 * Cloudflare Pages Middleware to enforce Firebase App Check on Admin API routes.
...
async function getJwk(kid) {
  ...
  if (!cachedJwks || now > cachedJwksExpiry) {
    console.log('[App Check Middleware Admin] Fetching public keys from Firebase...');
    ...
    if (response.ok) {
      ...
      console.error('[App Check Middleware Admin] Failed to fetch JWKS from Firebase');
    }
  }
...
export async function onRequest(context) {
  ...
  if (!isValid) {
    console.warn(`[App Check Middleware Admin] Validation failed: ${reason}`);
    
    if (isDev) {
      console.warn('[App Check Middleware Admin] Local development mode detected. Allowing request despite validation failure.');
      return next();
    }
  ...
  console.log('[App Check Middleware Admin] ✅ App Check verification successful.');
  return next();
}
