 ...
 * This is the PRIMARY defense against email spam loops and respects Resend quota limits.
 */
...
/**
 * Main Email Service Interface (Admin Panel)
 */
...
/**
 * Core email sender with deduplication and Resend limits protection
 */
...
/**
 * Automatically trigger order status change email to the customer
 * @param {Object} order - Full order object from Firestore
 * @param {string} status - New order status (Placed, Packed, Shipped, Delivered)
...
export const sendOrderStatusEmail = async (order, status) => {
  ...
  const recipientEmail = order.userEmail || order.email;
  if (!recipientEmail) {
    console.warn(`⚠️ emailService: No userEmail found for order ${order.orderId || order.id || ''}, skipping status email.`);
    return { success: false, error: 'No recipient email' };
  }

  if (!isEmailEnabled()) {
    console.log('📧 emailService: Email disabled via featureConfig, skipping.');
    return { success: false, error: 'Email disabled' };
  }

  // DEDUP GUARD: Prevent same status email from being triggered multiple times in rapid succession
  const orderId = order.orderId || order.id || '';
  const dedupKey = `order-status-${status.toLowerCase()}-${orderId}`;
  if (isDuplicateEmail(dedupKey)) {
    console.warn(`⚠️ emailService: BLOCKED duplicate status email for ${dedupKey}`);
    return { success: true, data: { deduplicated: true } };
  }

  try {
    const emailBody = generateOrderStatusHTML(order, status, order.tracking || {});
    const subject = generateEmailSubject(order, status.toLowerCase());
    ...
    console.error(`❌ emailService: Failed to send ${status} email:`, error);
    return { success: false, error: error.message };
  }
};
